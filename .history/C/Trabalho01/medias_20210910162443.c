...
int qtd_alunos, qtd_provas, opcao_menu = -1, peso_total_provas = 0, total_alunos = 0, lider = 0, vice_lider = 0;
char ch;
...
void checar_idade_aluno(Aluno alunos[])
{
	if (alunos[total_alunos].idade > alunos[lider].idade)
	{
		vice_lider = lider;
		lider = total_alunos;
	}
	else if (alunos[total_alunos].idade > alunos[vice_lider].idade && lider != vice_lider)
	{
		vice_lider = total_alunos;
	}
	printf("lider: %s e vice-lider: %s\n", alunos[lider].nome, alunos[vice_lider].nome);
}
