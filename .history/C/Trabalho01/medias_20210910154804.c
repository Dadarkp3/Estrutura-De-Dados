...
int qtd_alunos, qtd_provas, opcao_menu = -1, peso_total_provas = 0, total_alunos = 0, lider = -1, vice_lider = -1;
char ch;
...
void checar_lider_vice_lider(Aluno alunos[]);
...
int main()
{
	...
	do
	{
		...
		case 5:
			checar_lider_vice_lider(alunos);
			break;
...
void menu()
{
	...
	printf("1. Sair\n\n");
	printf("2. Adicionar um novo aluno a lista.\n");
	printf("3. Imprimir Relatório de Notas Alunos.\n");
	printf("4. Imprimir média das provas.\n");
	printf("5. Imprimir o nome do líder e do vice líder.\n");
	printf("/*****************************************/\n\n");
	reset();
...
void adicionar_aluno(Aluno alunos[], int pesos[], float medias_provas[])
{
	...
	else
	{
		...
		printf("Digite a idade do aluno: ", &alunos[total_alunos].idade);
...
void checar_lider_vice_lider(Aluno alunos[])
{
	...
	else
	{
		...
		while (getchar() != '\n')
			...
			{
				if (alunos[i].idade > lider)
				{
					vice_lider = i;
					lider = alunos[i].idade;
				}
				else if (alunos[i].idade > vice_lider)
				{
					vice_lider = i;
				}
			}
		if (vice_lider != -1)
		{
			printf("O líder da turma é: %s. Ainda não temos um vice líder.", alunos[lider].nome);
		}
		else
		{
			printf("O líder da turma é: %s. O vice líder da turma é: %s.", alunos[lider].nome, alunos[vice_lider].nome);
		}
	}
	sair_menu();
}
